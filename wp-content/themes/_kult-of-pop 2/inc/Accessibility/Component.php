...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'kult-of-pop-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			[],
			kult_of_pop()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'kult-of-pop-navigation', 'async', true );
		wp_script_add_data( 'kult-of-pop-navigation', 'precache', true );
		wp_localize_script(
			'kult-of-pop-navigation',
			'kultOfPopScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'kult-of-pop' ),
				'collapse' => __( 'Collapse child menu', 'kult-of-pop' ),
			]
		);
	}
