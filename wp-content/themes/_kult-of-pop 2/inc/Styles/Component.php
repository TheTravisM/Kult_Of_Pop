...
use function WP_Rig\WP_Rig\kult_of_pop;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'kult-of-pop-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = kult_of_pop()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'kult-of-pop-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'kult-of-pop' ), $handle ) ), 'Kult Of Pop 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'kult_of_pop_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! kult_of_pop()->is_amp();
		 ...
		 */
		return apply_filters( 'kult_of_pop_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			...
			'kult-of-pop-comments' => [
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			],
			'kult-of-pop-c-header' => [
				'file'             => 'components/c-header.min.css',
				'preload_callback' => '__return_true',
			],
			'kult-of-pop-c-header-nav' => [
				'file'             => 'components/c-header-nav.min.css',
				'preload_callback' => '__return_true',
			],
			'kult-of-pop-c-header-social-media' => [
				'file'             => 'components/c-header-social-media.min.css',
				'preload_callback' => '__return_true',
			],
			'kult-of-pop-c-footer' => [
				'file'             => 'components/c-footer.min.css',
				'preload_callback' => '__return_true',
			],
			'kult-of-pop-c-footer-nav' => [
				'file'             => 'components/c-footer-nav.min.css',
				'preload_callback' => '__return_true',
			],
			'kult-of-pop-c-footer-social-media' => [
				'file'             => 'components/c-footer-social-media.min.css',
				'preload_callback' => '__return_true',
			],
			'kult-of-pop-content' => [
			...
			'kult-of-pop-sidebar' => [
				...
				'preload_callback' => function() {
					return kult_of_pop()->is_primary_sidebar_active();
				},
			],
			'kult-of-pop-widgets' => [
		 ...
		 */
		$css_files = apply_filters( 'kult_of_pop_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'kult_of_pop_google_fonts', $google_fonts );
