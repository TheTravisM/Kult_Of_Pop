...
add_action( 'wp_enqueue_scripts', 'kult_of_pop_products_match_height', 99 );
/**
 * Prints an inline script to the footer to keep products the same height.
 *
 * @since 2.3.0
 */
function kult_of_pop_products_match_height() {
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'kult_of_pop_woocommerce_breakpoint' );
/**
 ...
 */
function kult_of_pop_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'kult_of_pop_default_products_per_page' );
/**
 ...
 */
function kult_of_pop_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'kult_of_pop_woocommerce_pagination' );
/**
 ...
 */
function kult_of_pop_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'kult_of_pop' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'kult_of_pop' ) );

	return $args;

}

add_action( 'after_switch_theme', 'kult_of_pop_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function kult_of_pop_woocommerce_image_dimensions_after_theme_setup() {
	...
	kult_of_pop_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'kult_of_pop_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function kult_of_pop_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function kult_of_pop_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'kult_of_pop_gallery_image_thumbnail' );
/**
 ...
 */
function kult_of_pop_gallery_image_thumbnail( $size ) {
