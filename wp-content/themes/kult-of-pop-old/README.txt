...
GitHub project link: https://github.com/studiopress/kult_of_pop/.
...
The version of [Genesis Sample on GitHub](https://github.com/studiopress/kult_of_pop/) includes tooling to check code against WordPress standards. To use it:
...
- `npm run makepot` to regenerate the `languages/kult_of_pop.pot` file.
...
- `npm run zip` to create a kult_of_pop.zip. Files in the `excludes` array in `scripts/makezip.js` are omitted.
...
4. Type `npm run zip` to create `kult_of_pop.zip`. Files in the `excludes` array in `scripts/makezip.js` are omitted from the zip. `filename.md` files will be renamed to `filename.txt`.
