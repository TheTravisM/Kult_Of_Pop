...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'kop-oct19-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			[],
			kop_oct19()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'kop-oct19-navigation', 'async', true );
		wp_script_add_data( 'kop-oct19-navigation', 'precache', true );
		wp_localize_script(
			'kop-oct19-navigation',
			'kopOct19ScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'kop-oct19' ),
				'collapse' => __( 'Collapse child menu', 'kop-oct19' ),
			]
		);
	}
